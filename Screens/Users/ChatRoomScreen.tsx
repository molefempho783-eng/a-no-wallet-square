...
const ChatRoomScreen = () => {
  ...
  useEffect(() => {
    ...
    const chatDocRef = doc(db, 'chats', chatId);
    const unsubPinned = onSnapshot(chatDocRef, (snap) => {
      if (!snap.exists()) {
        setPinnedMeta({});
        return;
      }
...
const handlePickFile = async () => {
    ...
    try {
      ...
      try {
        ...
        if (pinnedMeta.pinnedMessageId === messageId) {
          await unpinChatMessage(doc(db, 'chats', chatId)).catch(() => {});
        }
        setShowMessageOptions(false);
        setSelectedMessage(null);
  ...
  const handlePin = async () => {
    ...
    try {
      await pinChatMessage(doc(db, 'chats', chatId), selectedMessage, currentUser.uid);
      setShowMessageOptions(false);
      setSelectedMessage(null);
    } catch {
  ...
  const handleUnpin = async () => {
    try {
      await unpinChatMessage(doc(db, 'chats', chatId));
    } catch {
              ...
              <>
                ...
                >
                  <Text style={{ color: colors.text, fontSize: 16 }}>
                    {pinnedMeta.pinnedMessageId === selectedMessage.id ? 'Already pinned' : 'Pin message'}
                  </Text>
                </TouchableOpacity>
