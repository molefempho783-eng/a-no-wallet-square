...
export default function CreateMapSpecialScreen() {
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedPlace && !gpsCoords && !bias) {
      Alert.alert('Location unavailable', 'Search and pick a place, or use the locate button.');
      return;
    }
    ...
    try {
      ...
      if ((pinLat == null || pinLng == null) && gpsCoords) {
        pinLat = gpsCoords.lat;
        pinLng = gpsCoords.lng;
      }
    ...
    >
      ...
      >
        ...
        <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 10 }}>
          ...
          <TouchableOpacity
            style={[styles.locateBtn, { backgroundColor: colors.primary, marginTop: 28 }]}
            onPress={async () => {
              const bias = biasCoords();
              if (!bias) {
              ...
              try {
                ...
                setSelectedPlace(null);
                setGpsCoords({ lat: bias.lat, lng: bias.lng });
              } finally {
