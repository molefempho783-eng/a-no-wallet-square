...
export default function ActivityChatScreen() {
  ...
  useEffect(() => {
    if (!canAccess) return;
    const unsubPinned = onSnapshot(chatRef, (snap) => {
      if (!snap.exists()) {
        setPinnedMeta({});
        return;
      }
  ...
  const deleteMessage = async (messageId: string) => {
    if (!currentUserId) return;
    try {
      await deleteDoc(doc(db, 'activityChats', activityId, 'messages', messageId));
      if (pinnedMeta.pinnedMessageId === messageId) {
        await unpinChatMessage(chatRef).catch(() => {});
      }
    } catch {
      Alert.alert('Could not delete', 'Unable to delete this message.');
    }
    setShowMessageOptions(false);
    setSelectedMessage(null);
  };
  ...
  const handlePin = async () => {
    ...
    try {
      await pinChatMessage(chatRef, selectedMessage, currentUserId);
      setShowMessageOptions(false);
      setSelectedMessage(null);
    } catch {
      Alert.alert('Could not pin', 'Unable to pin this message.');
    }
  };

  const handleUnpin = async () => {
    try {
      await unpinChatMessage(chatRef);
    } catch {
      Alert.alert('Could not unpin', 'Unable to remove the pin.');
    }
  };
      ...
      >
        ...
        >
          ...
          >
            {selectedMessage && (
              <>
                {selectedMessage.text?.trim() ? (
                  <TouchableOpacity style={localStyles.modalOption} onPress={handleCopy}>
                    <Text style={{ color: colors.text, fontSize: 16 }}>Copy</Text>
                  </TouchableOpacity>
                ) : null}
                <TouchableOpacity style={localStyles.modalOption} onPress={handlePin}>
            ...
            <TouchableOpacity
              style={[localStyles.modalOption, { marginTop: 8 }]}
              onPress={() => {
                setShowMessageOptions(false);
                setSelectedMessage(null);
              }}
